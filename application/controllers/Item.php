...
class item extends CI_Controller
{
    ...
    function get_ajax()
    {
        ...
        foreach ($list as $item) {
            ...
            $row[] = $item->unit_name;
            $row[] = $this->fungsi->indo_currency($item->price);
            $row[] = $item->stock;
    ...
    public function process()
    {
        ...
        if (isset($_POST['add'])) {
            if ($this->item_m->check_barcode($post['barcode'])->num_rows() > 0) {
                $this->session->set_flashdata('error', "Barcode $post[barcode] sudah diapakai orang lain");
                redirect('item/add');
            } else {
            ...
            if ($this->item_m->check_barcode($post['barcode'], $post['id'])->num_rows() > 0) {
                ...
                if (@$_FILES['image']['name'] != null) {
                    if ($this->upload->do_upload('image')) {
                        //replace image agar memori tidak cepat penuh
